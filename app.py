...
class convertor(ctk.CTk):
    def __init__ (self):
        ...
        self.help_button = ctk.CTkButton(self.header, text="Help", command = lambda: self.switch_frame(frames.LengthFrame))
        self.help_button.grid(column = 0, row = 0, pady = pady_m, padx = padx_m, sticky = "nsew")
        self.options_button = ctk.CTkButton(self.header, text="Options", command = lambda: self.switch_frame(frames.LengthFrame))
        ...
        self.wm_button = ctk.CTkButton(self.tabs, text="weight and mass", command = lambda: self.switch_frame(frames.LengthFrame))
        self.wm_button.grid(column = 4, row = 0, pady = pady_m, padx = padx_m)
        self.temp_button = ctk.CTkButton(self.tabs, text="temperature", command = lambda: self.switch_frame(frames.LengthFrame))
        self.temp_button.grid(column = 5, row = 0, pady = pady_m, padx = padx_m)
        self.energy_button = ctk.CTkButton(self.tabs, text="energy", command = lambda: self.switch_frame(frames.LengthFrame))
        self.energy_button.grid(column = 6, row = 0, pady = pady_m, padx = padx_m)
        self.speed_button = ctk.CTkButton(self.tabs, text="speed", command = lambda: self.switch_frame(frames.LengthFrame))
        self.speed_button.grid(column = 7, row = 0, pady = pady_m, padx = padx_m)
        self.time_button = ctk.CTkButton(self.tabs, text="time", command = lambda: self.switch_frame(frames.LengthFrame))
        self.time_button.grid(column = 8, row = 0, pady = pady_m, padx = padx_m)
        self.power_button = ctk.CTkButton(self.tabs, text="power", command = lambda: self.switch_frame(frames.LengthFrame))
        self.power_button.grid(column = 9, row = 0, pady = pady_m, padx = padx_m)
        self.data_button = ctk.CTkButton(self.tabs, text="data", command = lambda: self.switch_frame(frames.LengthFrame))
        self.data_button.grid(column = 10, row = 0, pady = pady_m, padx = padx_m)
        self.pressure_button = ctk.CTkButton(self.tabs, text="pressure", command = lambda: self.switch_frame(frames.LengthFrame))
        self.pressure_button.grid(column = 11, row = 0, pady = pady_m, padx = padx_m)
        self.angle_button = ctk.CTkButton(self.tabs, text="angle", command = lambda: self.switch_frame(frames.LengthFrame))
